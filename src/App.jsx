...
function PieChart({ income, expense, canvasRef }) {
  ...
  return <canvas ref={draw} style={{ display: "block", margin: "0 auto" }} />;
}
...
export default function App() {
  ...
  async function recargarDatos() {
    try {
      ...
      setEntradas((entradasLista || []).map(normalizarTransaccion));
      setSalidas((salidasLista || []).map(normalizarTransaccion));
    } catch (error) {
  ...
  async function submitEntrada() {
    ...
    try {
      ...
      console.error("Error completo al guardar entrada:", error);
  ...
  async function submitSalida() {
    ...
    try {
      ...
      console.error("Error completo al guardar salida:", error);
      console.error("Status:", error?.response?.status);
      console.error("Respuesta del backend:", error?.response?.data);
        ...
        <div className="balance-grid">
          <div className="balance-table-wrap">
            ...
            <table>
              ...
              <tbody>
                {entradas.map((e) => (
                  <tr key={e.id}>
                    <td>{e.tipo}</td>
                    <td style={{ color: "var(--green)" }}>{fmt(e.monto)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            <div className="balance-total">
              <span>TOTAL</span>
              <span style={{ color: "var(--green)" }}>{fmt(totalIn)}</span>
            </div>
          </div>
          ...
          <div className="balance-table-wrap">
            ...
            <table>
              ...
              <tbody>
                {salidas.map((s) => (
                  <tr key={s.id}>
                    <td>{s.tipo}</td>
                    <td style={{ color: "var(--red)" }}>{fmt(s.monto)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            <div className="balance-total">
              <span>TOTAL</span>
              <span style={{ color: "var(--red)" }}>{fmt(totalOut)}</span>
            </div>
          </div>
        </div>
